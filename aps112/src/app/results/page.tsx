    ...
    <div className="glass-card rounded-xl overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm text-gray-200 leading-snug">"{result.claim.originalText}"</p>
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${s.badge}`}>{s.label}</span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-blue-900/50 text-blue-300">{result.claim.subject}</span>
            {result.matchedRule && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-white/10 text-gray-400 font-mono">{result.matchedRule.sectionReference}</span>
            )}
            {result.matchedRule?.severity === 'Critical' && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-red-900/60 text-red-400 font-medium">Critical</span>
            )}
          </div>
        </div>
        <svg className={`w-4 h-4 text-[#8e8ea0] shrink-0 mt-0.5 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="px-4 pb-4 border-t border-white/10">
          ...
          {result.matchedRule && (
            <div className="mt-3 rounded-lg glass-card px-3 py-2">
              ...
              <p className="text-xs text-[#8e8ea0] mt-1">{result.matchedRule.value}</p>
            </div>
          )}
  ...
  if (refMap.size === 0) {
    return (
      <div className="glass rounded-2xl p-4">
        <h3 className="text-xs font-semibold text-[#8e8ea0] uppercase tracking-wider mb-3">OFC References</h3>
  ...
  return (
    <div className="glass rounded-2xl p-4 space-y-4">
      ...
      {Array.from(bySource.entries()).map(([source, refs]) => (
        <div key={source}>
          <p className="text-xs font-medium text-white/70 mb-2 leading-tight">{source}</p>
          <div className="space-y-1.5">
            {Array.from(refs.entries())
              .sort((a, b) => a[0].localeCompare(b[0]))
              .map(([sectionRef, { rule, statuses }]) => (
                <div key={sectionRef} className="flex items-start gap-2 rounded-lg bg-white/5 px-2.5 py-2">
                  <span className={`inline-block w-2 h-2 rounded-full ${statusDotColor(statuses)} shrink-0 mt-1`} />
                  <div className="min-w-0">
                    <p className="text-xs font-mono font-medium text-white">{sectionRef}</p>
                    <p className="text-xs text-[#8e8ea0] mt-0.5 leading-snug">{rule.subject}</p>
                  </div>
                </div>
              ))}
          </div>
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-start gap-8">
              ...
              <div className="flex-1">
                <h2 className="text-base font-semibold text-white mb-1">Compliance score</h2>
                <p className="text-xs text-[#8e8ea0] mb-4">{report.results.length} claims checked · {checkedAt}</p>
                ...
                {report.criticalConflicts > 0 && (
                  <div className="mt-3 flex items-center gap-1.5 rounded-lg bg-red-950/50 border border-red-800/60 px-3 py-2">
                    <svg className="w-3.5 h-3.5 text-red-400" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd"/>
                    </svg>
                    <p className="text-xs font-medium text-red-300">{report.criticalConflicts} critical conflict{report.criticalConflicts !== 1 ? 's' : ''} detected</p>
                  </div>
                )}
              </div>
            </div>
